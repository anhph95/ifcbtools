...
from ifcb.neslter.community_variability import make_community_array
...
def repo_root() -> Path:
    """Return the repository root from analysis/community-variability/scripts."""
    return REPO_ROOT
...
def load_ifcb_carbon(data_dir: Path, data_version: str = "filled") -> tuple[pd.DataFrame, list[str]]:
    """Load IFCB carbon biomass and identify taxon biomass columns j."""
    if data_version not in {"clean", "filled"}:
        raise ValueError("data_version must be 'clean' or 'filled'.")

    carbon_path = data_dir / f"ifcb_carbon_{data_version}.csv"
    taxonomy_path = data_dir / f"ifcb_taxonomy_{data_version}.csv" if data_version == "filled" else data_dir / "ifcb_taxonomy.csv"
