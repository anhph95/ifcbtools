"""Command-line interface for the IFCB processing pipeline."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Process NES-LTER IFCB raw metadata, count, and carbon CSV files.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("input_data_path", help="Directory containing IFCB input CSV files.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    LOGGER.info("Pipeline completed. Outputs: %s", ", ".join(str(path) for path in outputs))
    return 0
